...
function snapshot(over = {}) {
  return { state: 'running', score: 412, distance: 287.6, crops: 5, lives: 2, ...over };
}

describe('page', () => {
  it('carries every element the HUD writes to', async () => {
    const page = await loadPage();
    for (const id of ['stage', 'score', 'distance', 'crops', 'shells', 'overlay', 'overlay-kicker',
...
describe('hud', () => {
  it('writes the run onto the readouts', async () => {
    ...
    assert.equal(page.$('#distance').textContent, '287m');
    assert.equal(page.$('#crops').textContent, '5');
  ...
  it('opens on the sign, and hides the best score until there is one', async () => {
    const page = await loadPage();
    const hud = createHud(page.document, 3);
    ...
    assert.equal(page.$('#play').textContent, 'run for it');
...
describe('best', () => {
  ...
  it('ignores junk left in storage', () => {
    assert.equal(readBest(fakeStorage({ 'eggscape-farm:best': 'rise and shine' })), 0);
    assert.equal(readBest(fakeStorage({ 'eggscape-farm:best': '-5' })), 0);
  });
});
