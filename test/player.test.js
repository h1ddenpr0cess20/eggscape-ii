...
/** A field of exactly one plot, so the physics has something to stand on. */
...
/** A plot that only ends at `at` — past its far edge there is only the cut. */
...
describe('player', () => {
  ...
  it('settles onto the plot it starts over', () => {
    const course = floor();
    const player = createPlayer();
  ...
  it('falls off the side of a narrow plot', () => {
  ...
  it('does not get caught on the lip of a plot it came up short against', () => {
    const player = createPlayer({ y: 0 });
    const raised = floor({ y: SNAP + 0.4, z0: 4, z1: 200 });
    player.vy = -1;
    settle(player, raised, 0.4);
    assert.ok(!player.grounded, 'it snapped up through the front of the plot');
    assert.ok(player.y < 0);
  });
  ...
  it('slams down when told to, and only in the air', () => {
    ...
    settle(player, course, 0.2);
    assert.ok(player.grounded, 'a slam should reach the soil fast');
    assert.ok(height > 0.5, 'the test should have slammed from a height');
  });

  it('goes under eventually, and the cut is the only way to lose height', () => {
  ...
  it('puts a fallen egg back on a plot, in a lane that plot has', () => {
