...
describe('the box builder', () => {
   ...
   *  from within — a fence you can only see from inside the field. */
  ...
  it('bakes the tiling into the uvs, so one texture serves every plot', () => {
    ...
    assert.equal(u, 6, 'the soil would be stretched across the plot instead of tiled');
    assert.equal(v, 15);
  });
});
