...
import {
  AIRTIME, BALE, FALL_Y, INVULNERABLE, LANES, laneX, LIVES, SCORE,
...
describe('game', () => {
  ...
  it('runs once started, and scores the ground it covers', () => {
    ...
    assert.equal(snapshot.score, Math.floor(snapshot.distance * SCORE.perMetre) + snapshot.crops * SCORE.perCrop);
  });
  ...
  it('pays for a crop once', () => {
    const game = started(2);
    let taken = 0;
    game.on('crop', () => { taken += 1; });
    const snapshot = play(game, 12, null);
    assert.equal(snapshot.crops, taken);
    assert.equal(game.course.crops.filter((crop) => crop.taken).length + countGone(game), taken);

    function countGone() {
      return taken - game.course.crops.filter((crop) => crop.taken).length;
    }
  });

  it('costs a shell to meet a bale, and gives a moment of grace after', () => {
    const game = started(1);
    const bale = { z: game.player.z + 30, lane: 1, x: 0, y: 0, kind: 0, hit: false };
    game.course.bales.length = 0;
    game.course.bales.push(bale, { ...bale, z: bale.z + 1.2 });

    const hits = [];
    game.on('hit', (hit) => hits.push(hit));
    play(game, 4);

    assert.equal(hits.length, 1, 'the second bale should have been passed through');
    assert.equal(hits[0].reason, 'bale');
  ...
  /** One bale, dropped on the opening headland in the egg's own lane. */
  function ambush(game, ahead = 12) {
    ...
    game.course.bales.length = 0;
    game.course.bales.push(bale);
    return bale;
  }

  it('bursts a bale slammed onto, and pays for it', () => {
    const game = started(1);
    const bale = ambush(game);

    const burst = [];
    game.on('burst', (event) => burst.push(event));
    game.on('hit', () => assert.fail('a slammed bale cost a shell'));

    /** Hop in time to be over it, then put the slam on and ride it down. */
    const reach = () => (bale.z - game.player.z) / game.player.speed;
    ...
    assert.equal(burst.length, 1, 'the slam went straight past it');
    assert.ok(bale.hit);
    assert.equal(game.snapshot().lives, LIVES);
    assert.equal(game.snapshot().bales, 1);
    assert.ok(game.score >= SCORE.perBale, 'bursting a bale paid nothing');
  });
  ...
  it('still costs a shell to run into one on the ground', () => {
    ...
    const hits = [];
    game.on('burst', () => assert.fail('a flat run-in burst a bale'));
    ...
    assert.deepEqual(hits, ['bale'], 'holding slam on the ground walked through it');
  });
  ...
  it('only reaches the lane it came down in', () => {
    ...
    const bale = ambush(game);
    bale.lane += 1;
    bale.x = laneX(bale.lane);

    game.on('burst', () => assert.fail('the slam reached into the next lane'));
    const reach = () => (bale.z - game.player.z) / game.player.speed;
    while (game.state === 'running' && reach() > AIRTIME * 0.75) game.advance(1 / 120, null);
    game.advance(1 / 120, { left: 0, right: 0, jump: true, dive: false });
    while (game.state === 'running' && !game.player.grounded) {
  ...
  it('puts a fallen egg back down and says where', () => {
    ...
    assert.ok(moves[0].seg, 'the plot it was put on should come with the event');
    assert.ok(game.player.z >= moves[0].seg.z0);
    assert.equal(game.state, 'running');
  });

  it('lets a hop clear a bale', () => {
    const game = started(1);
    game.course.bales.length = 0;
    game.course.bales.push({ z: game.player.z + 14, lane: 1, x: 0, y: 0, kind: 0, hit: false });

    const hits = [];
    game.on('hit', (hit) => hits.push(hit.reason));

    /** Hopped from four metres out the arc carries the whole bale; from
     *  seven it is already coming down on the far edge of one. */
    const bale = game.course.bales[0];
    while (game.player.z < bale.z - 4.2) game.advance(1 / 120, null);
    game.advance(1 / 120, { left: false, right: false, jump: true, dive: false });
    while (game.player.z < bale.z + 2 && game.state === 'running') game.advance(1 / 120, null);

    assert.ok(!hits.includes('bale'), `a bale ${BALE.height}m tall stopped a hop`);
    assert.ok(game.player.z > bale.z, 'the egg never reached the bale');
  });
  ...
  it('costs a shell to fall, and puts the egg back down still running', () => {
    ...
    assert.equal(game.state, 'running');
    assert.ok(game.player.y > FALL_Y, 'the egg was left down the bank');
  ...
  it('starts clean every time, on a course of its own', () => {
    ...
    assert.equal(game.snapshot().crops, 0);
  ...
  it('keeps the field to a working set however far the egg gets', () => {
    const game = started(3);
    play(game, 40);
    assert.ok(game.course.segments.length < 60, `${game.course.segments.length} plots still live`);
  ...
  it('is survivable: a crude autopilot gets a long way down every seed', () => {
    for (const seed of SEEDS) {
      ...
      assert.ok(result.crops > 5, `seed ${seed} gave up only ${result.crops} crops`);
    }
  });
});
